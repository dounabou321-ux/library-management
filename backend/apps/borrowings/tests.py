...
def test_borrow_and_return_updates_stock():
    user = User.objects.create_user(email="member@example.com",
                                    password="pass12345", first_name="Test", last_name="Member")
    ...
    book = Book.objects.create(title="A Wizard of Earthsea", author=author,
                               category=category, copies_total=1, copies_available=1)
